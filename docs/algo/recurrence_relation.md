...
>
> 즉, 재귀적 알고리즘에서 분할된 문제의 개수가 a개이고, 입력의 크기를 b로 나누는 경우, 각 레벨에서의 **작업량은 a배씩 증가**하고, **문제의 크기는 b배씩 감소**한다. 상위 레벨에서 하위 레벨로 내려갈 때 `문제의 크기`와 `분할된 문제의 개수`가 어떻게 `상쇄`되는지를 나타낸다.  
>
...
>
> $n^{\log_b{a}}$와 $f(n)$을 비교하여 전체 시간 복잡도를 결정할 수 있다.
>
> 1. $f(n) = O(n^{\log_b{a} - \epsilon})$ 인 경우 ($\epsilon > 0$), $T(n) = \Theta(n^{\log_b{a}})$  
> 2. $f(n) = O(n^{\log_b{a}})$ 인 경우, $T(n) = \Theta(n^{\log_b{a}}\log{n})$  
> 3. $f(n) = O(n^{\log_b{a} + \epsilon})$ 인 경우 ($\epsilon > 0$), $T(n) = \Theta(f(n))$
...
3. ${f(n)\over{h(n)}}=\Theta(1)$이면, $T(n)=\Theta(h(n)\cdot{log{n}})$이다.

> 1. 트리의 각 레벨에서 노드의 수는 $a^i$로 증가  
> 2. 노드당 작업량은 $f(\frac{n}{b^i})$로 감소  
> 3. 각 레벨에서의 전체 작업량은 $a^i f(\frac{n}{b^i})$  
> 4. 레벨의 수는 $\log_b{n}$  
>
> 전체 시간 복잡도:  
...
>
> 2. $f(n) = O(n^{\log_b{a}})$:  
> 이 경우, 각 레벨에서의 작업량이 대략 비슷하므로 전체 시간 복잡도는 레벨 수에 비례한다.  
> 전체 시간 복잡도는 $T(n) = \Theta(n^{\log_b{a}}\log{n})$
>
>
> 3. $f(n) = O(n^{\log_b{a} + \epsilon})$ 인 경우 ($\epsilon > 0$):  
> 하위 레벨로 갈수록 작업량이 지수적으로 증가하므로 하위 레벨에서 대부분의 작업이 이루어진다. 이 경우에는 $f(n)$의 상대적인 중요도가 더 커진다.  
> 전체 시간 복잡도는 $T(n) = \Theta(f(n))$

2번 항목 $f(n) = O(n^{\log_b{a}})$의 경우는 각 레벨에서의 작업량이 대략 비슷하다. 이는 상위 레벨에서 하위 레벨로 내려가면서 *문제의 크기가 b배씩 감소*하고, *분할된 문제의 개수가 a배씩 증가*하는데, 이 둘 간의 상쇄가 발생하기 때문이다. 이 경우, $f(n)$와 $n^{\log_b{a}}$가 같은 차수(degree)를 갖게 된다.

그러므로 각 레벨에서의 작업량이 대략 비슷하므로 전체 시간 복잡도는 레벨 수에 비례하게 된다. 이 때

1. 레벨 수는 $\log_b{n}$이고,
2. 각 레벨에서의 작업량은 대략 $n^{\log_b{a}}$

따라서 전체 시간 복잡도는 레벨 수와 각 레벨에서의 작업량을 곱한 값인 $T(n) = \Theta(n^{\log_b{a}}\log{n})$이 된다.

간단한 예로 병합 정렬의 경우 $a = 2, b = 2$이고, $f(n) = n$이다. 여기서 $n^{\log_b{a}} = n^{\log_2{2}} = n^1 = n$이므로, 각 레벨에서의 작업량이 대략 비슷하다. 이때 전체 시간 복잡도는 $T(n) = \Theta(n\log{n})$이 된다.
